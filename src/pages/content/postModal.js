...
import { createContentCreatorContractInstance, createUserContractInstance } from "../../services/contracts_creators";
...
const PostModal = ({ closeModal, destination, community_id }) => {
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            console.log('content_id from getDraft() =>', content_id);
            ...
            const community_id = postDestination === 'Your feed' ? 0 : community_id || 0;
            const date = Math.floor(new Date().getTime() / 1000);

            const duplicate_contentData = `{"content_id":"${content_id+1n+''}","author":"${contract.address}","sub_data":"${stringifiedData}","content":"","community_id":"${community_id}","verified":false,"timestamp":${date}}`;
